  ...
  return (
    <div
      className={`flex gap-4 rounded-sm ${
        user.id === userId && 'hover:bg-gray-50'
      } ${!merge && 'mt-2'}`}
    ...
    >
        ...
        <div>
          {edit ? (
            ...
            >
              ...
              {edited && (
                <span
                  title={`Edited: ${dayjs(updatedAt).calendar()}`}
                  className="ml-2 italic text-xs text-gray-400"
                >
                  edited
                </span>
              )}
      ...
      {user.id === userId && hovering && !edit && (
        <MessageButtons id={id} setEdit={setEdit} />
      )}
    </div>
  );
}
